...
public class LoadProfileUpdate extends AsyncTask<String,String,String> {
    ...
    String suc = "";
    String msg = "", nm,nmM,nmL,email,password, phone, address, imagePath;
...
//            conn.setRequestProperty("user_image", sourceFile.getAbsolutePath());
            ...
            dos.writeBytes(twoHyphens + boundary + lineEnd);
            dos.writeBytes("Content-Disposition: form-data; name=nameMid;" + lineEnd);
            dos.writeBytes(lineEnd);
            dos.write(nmM.getBytes("UTF-8"), 0, nmM.getBytes("UTF-8").length);
            dos.writeBytes(lineEnd);

            dos.writeBytes(twoHyphens + boundary + lineEnd);
            dos.writeBytes("Content-Disposition: form-data; name=nameLast;" + lineEnd);
            dos.writeBytes(lineEnd);
            dos.write(nmL.getBytes("UTF-8"), 0, nmL.getBytes("UTF-8").length);
            dos.writeBytes(lineEnd);
...
//            String serverResponseMessage = conn.getResponseMessage();
            ...
            try {
                ...
                String nameMid = jsonObject.getString(Constant.TAG_USER_NAME_MID);
                String nameLast = jsonObject.getString(Constant.TAG_USER_NAME_LAST);
                ...
                Constant.itemUser = new ItemUser(id,name,nameMid,nameLast,email,mobile,image,address);
